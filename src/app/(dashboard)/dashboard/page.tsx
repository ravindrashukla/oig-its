          ...
          <CardContent>
            <div className="flex flex-wrap gap-3">
              {Object.entries(metrics.casesByStatus).map(([status, count]) => (
                <div
                  key={status}
                  className={`inline-flex items-center gap-2 rounded-md px-3 py-1.5 text-xs font-medium ${statusColor[status] ?? "bg-muted text-muted-foreground"}`}
                >
            ...
            {casesLoading ? (
              ...
              recentCases?.data.map((c) => (
                ...
                >
                  ...
                  <span
                    className={`shrink-0 rounded px-2 py-0.5 text-[10px] font-medium ${statusColor[c.status] ?? ""}`}
                  >
        ...
        <CardContent className="space-y-1">
          {notificationsLoading ? (
            ...
            notifications.map((n) => (
              ...
              >
                <Bell
                  className={`mt-0.5 size-4 shrink-0 ${n.isRead ? "text-muted-foreground" : "text-blue-600"}`}
                />
                <div className="min-w-0 flex-1">
                  <p
